...
if args.reserve_vram is not None:
    ...
    print("Reserving {}MB vram for other applications.".format(EXTRA_RESERVED_VRAM / (1024 * 1024)))
    ...
    if not new_or_replaced_model and not force_patch_weights and not force_full_load:
        ...
        for loaded_model in models_to_load:
            if loaded_model.should_reload_model(force_patch_weights):
                can_reuse_loaded_models = False
                break
            if loaded_model.model_offloaded_memory() > 0:
                can_reuse_loaded_models = False
                break
            if loaded_model.device != torch.device("cpu") and get_free_memory(loaded_model.device) < minimum_memory_required:
                can_reuse_loaded_models = False
                break

        if can_reuse_loaded_models:
            return

    if new_or_replaced_model:
        soft_empty_cache(force=True)
...
def get_offload_stream(device):
    ...
    if device in STREAMS:
        ...
        for k in range(NUM_STREAMS):
            ss.append(torch.cuda.Stream(device=device, priority=9))
        STREAMS[device] = ss
        s = ss[stream_counter]
        stream_counter = (stream_counter + 1) % len(ss)
        return s
...
def soft_empty_cache(force=False):
    global cpu_state
    if not force and vram_state == VRAMState.HIGH_VRAM:
        return
