...
def _sdxl_cfg_rescale_post_cfg(cfg_result, cond_pred, cond_scale):
    ...
    cfg_std = cfg_result.float().std(dim=dims, keepdim=True)
    cond_std = cond_pred.float().std(dim=dims, keepdim=True)
    cfg_rescaled = cfg_result * (cond_std / cfg_std.clamp_min(1e-6)).to(cfg_result.dtype)

    return cfg_rescaled * rescale + cfg_result * (1.0 - rescale)
...
def normal_scheduler(model_sampling, steps, sgm=False, floor=False):
    ...
    logging.info(f"[Normal Scheduler Debug]   multiplier: {getattr(s, 'multiplier', 'N/A')}")
    ...
    logging.info(f"[Normal Scheduler Debug]   Generated {len(sigs)} sigmas, first 3: {sigs[:3]}, last 3: {sigs[-3:]}")
