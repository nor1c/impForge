...
def get_next_sequence_number(path, basename):
    ...
    """
    cache_key = (os.path.abspath(path), basename)
    cached = _sequence_number_cache.get(cache_key)
    if cached is not None:
        _sequence_number_cache[cache_key] = cached + 1
        return cached
    ...
    _sequence_number_cache[cache_key] = next_number + 1
    return next_number
...
def save_image_with_geninfo(image, geninfo, filename, extension=None, existing_pnginfo=None, pnginfo_section_name='parameters'):
    ...
    if extension.lower() == '.png':
        ...
        image.save(filename, format=image_format, quality=opts.jpeg_quality, pnginfo=pnginfo_data, compress_level=0)
...
def save_image(image, path, basename, seed=None, prompt=None, extension='png', info=None, short_filename=False, no_prompt=False, grid=False, pnginfo_section_name='parameters', p=None, existing_info=None, forced_filename=None, suffix="", save_to_dirs=None):
    ...
    if forced_filename is None:
        ...
        if add_number:
            ...
            fullfn = None
            for i in range(500):
                selected_number = basecount + i
                fn = f"{selected_number:05}" if basename == '' else f"{basename}-{selected_number:04}"
                fullfn = os.path.join(path, f"{fn}{file_decoration}.{extension}")
                if not os.path.exists(fullfn):
                    break
            if selected_number is not None:
                cache_key = (os.path.abspath(path), basename)
                _sequence_number_cache[cache_key] = max(_sequence_number_cache.get(cache_key, 0), selected_number + 1)
        else:
